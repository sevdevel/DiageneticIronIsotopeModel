...
NUMBER <- 5
BASENAME <- paste(NUMBER,"Sens BIOMIX Transient")
#=============================================================================
...
if (is.local){source("../Model_CSFe_FeisoBASE_localversion_final.R")}
#=============================================================================
...
PL$irr.0   <- 0.*290.0#*(0.5+0.5*loc.erf((O2.ow.sens-20.0)/12.0)) # irrigation rate at SWI [yr-1]
PL$irr.att <- 1.4 
PL$irr.inf <- 0.0
...
#PL$simulation.type <- "direct.steady.state"
PL$k.ISP     <- 1E+03
PL$k.PyP.1   <- 0.0*3.25 #1E+02 # -> Dale et al., 2015 - 3.25 (is from myself)
PL$k.PyP.2   <- 0.0*3.25 #1E+02
PL <- initialise.parameters(PL)
# Initialise set of state variables
PL <- initialise.state.variables(PL)
# Create and initialize object structures
...
# Initialisation simulation type 
sim.info$index <- 1
sim.info$code <- paste(NUMBER,"Sens BIOMIX 200530 SteadySta")
sim.info$name <- paste(sim.info$code,sim.info$index)
# Initialisation parameter list
sim.start <- simulation.list[[length(simulation.list)]]
PL <- sim.start$PL
