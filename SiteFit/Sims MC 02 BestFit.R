...
is.local <- TRUE
FOLDER   <- "MCrun"
BASENAME <- "01 MC Transient BestFit"
...
if (is.local){source("../Model_CSFe_Feiso_localversion_v01b.R")}
...
load("02 MC SteadySta BestFit 1.Rdata")

# Initialisation simulation type 

sim.info$index <- 1
sim.info$code <- "02 MC Transient BestFitb"
...
PL$a56Fe_PyP     <- (1.0 - 0.4/1000.0) # alpha for Pyrite precipitation
...
PL$d56Fe_FeOOH.f  <- -0.5
PL$d56Fe_FeOOH.a  <- -0.5
...
load("02 MC Transient BestFitb 1.Rdata")

# Initialisation simulation type 

sim.info$index <- 1
sim.info$code <- "02 MC SteadySta BestFit"
sim.info$name <- paste(sim.info$code,sim.info$index)
