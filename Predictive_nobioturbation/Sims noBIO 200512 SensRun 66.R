...
NUMBER <- 66
BASENAME <- paste(NUMBER,"Sens noBIO Transient")
#=============================================================================
...
PL$Db.0   <- 0.0*10.0*(0.5+0.5*loc.erf((O2.ow.sens-20.0)/12.0))
...
PL$irr.0   <- 0.0*290.0*(0.5+0.5*loc.erf((O2.ow.sens-20.0)/12.0)) # irrigation rate at SWI [yr-1]
PL$irr.att <- 1.4 
PL$irr.inf <- 0.0
...
# Initialisation simulation type 
sim.info$index <- 1
sim.info$code <- paste(NUMBER,"Sens noBIO 200512 SteadySta")
sim.info$name <- paste(sim.info$code,sim.info$index)
# Initialisation parameter list
sim.start <- simulation.list[[length(simulation.list)]]
PL <- sim.start$PL
